...
async function comprehensiveFilterTest() {
  ...
  try {
    ...
    const initialMessage = await page.textContent(
    ...
    if (!starSelect) starSelect = await page.$("select.form-select");
    if (!starSelect) throw new Error("Star select not found");
    ...
    for (let stars = 5; stars >= 1; stars--) {
      ...
      const starMessage = await page.textContent(
        '[data-testid="filter-status"]'
      );
      ...
      if (starReviews.length > 0) {
        const firstReviewStars = await page.$$eval(
          ".card:first-child .text-warning.fill-current",
          (stars) => stars.length
        );
    ...
    dateMessage = await page.textContent('[data-testid="filter-status"]');
    console.log(
      `    Results: ${dateFilteredReviews.length} reviews (should be 0)`
    );
    ...
    dateFilteredReviews = await page.$$(".card");
    console.log(
      `    Results with only start date: ${dateFilteredReviews.length} reviews`
    );
    ...
    dateFilteredReviews = await page.$$(".card");
    console.log(
      `    Results with only end date: ${dateFilteredReviews.length} reviews`
    );
    ...
    let combinedMessage = await page.textContent(
      '[data-testid="filter-status"]'
    );
    ...
    console.log(
      `    Results with spaces: ${spacesResults.length} reviews (should show all)`
    );
    ...
    console.log(
      `    Results with invalid date range: ${invalidDateResults.length} reviews`
    );
    ...
    const finalMessage = await page.textContent(
      '[data-testid="filter-status"]'
    );
