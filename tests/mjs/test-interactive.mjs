...
async function testReviewsPage() {
  ...
  try {
    ...
    const filterMessage = await page.textContent(
    ...
    console.log(
      `  - Visible reviews after search: ${visibleReviewsAfterSearch.length}`
    );
    ...
    console.log("\n⭐ Test 3: Star Rating Filter");
    // Ensure quick filters are expanded on mobile
    const quickFiltersToggle = await page.$('button:has-text("Quick filters")');
    if (quickFiltersToggle) {
      await quickFiltersToggle.click().catch(() => {});
      await delay(300);
    }

    // Prefer the canonical filters class used in the app
    let starSelect = await page.$("select.filters-control");
    if (!starSelect) {
      // Fallback to any form-select if class names change
      starSelect = await page.$("select.form-select");
    }

    if (!starSelect) {
      throw new Error("Star rating select not found");
    }
    ...
    // Test 4: Sort order (skip if control not present in current UI)
    console.log("\n📅 Test 4: Sort Order");
    const sortButton = await page.$(
      'button:has-text("Newest First"), button:has-text("Oldest First")'
    );
    if (sortButton) {
      ...
      console.log("  - Sort control not present, skipping");
    }
    ...
    const combinedFilterMessage = await page.textContent(
      '[data-testid="filter-status"]'
    );
    console.log(
      `  - Combined filter results: ${combinedFilterReviews.length} reviews`
    );
    ...
    console.log("\n🧹 Test 6: Clear Filters");
    const clearButton =
      (await page.$('button:has-text("Clear All")')) ||
      (await page.$('button:has-text("Clear")'));
    ...
    console.log(
      `  - Search field: "${searchValueAfterClear}" (should be empty)`
    );
    console.log(`  - Star filter: "${starValueAfterClear}" (should be empty)`);

    // Test 7: Reply button
    console.log("\n💬 Test 7: Reply Functionality");
    const firstNewReview = await page
      .$('.card:has([data-testid="reviews-list"]) button:has-text("Reply")')
      .catch(() => null);
    ...
    if (firstNewReview) {
      ...
      if (modalVisible) {
        const closeButton = await page.$(
          '.modal .btn-close, .modal button:has-text("Cancel")'
        );
        if (closeButton) {
    ...
    for (const test of testSearches) {
      ...
      const highlights = await page.$$("mark");
      console.log(
        `  - ${test.description} ("${test.term}"): ${results.length} results, ${highlights.length} highlights`
      );
    ...
    if (highlightElements.length > 0) {
      ...
      console.log(
        "  - Highlight styles:",
        JSON.stringify(firstHighlightStyle, null, 2)
      );
    }
